...
The general form of a time-independent Schr{\"o}dinger equation is shown in Eq. \ref{TISE}, where H is the Hamiltonian operator used to determine the total energy of a system with the wavefunction $\Psi$. Lowercase \textbf{r} are the electronic coordinates and $\sigma$ is used to denote the spin of electrons, which was omitted in earlier equations for simplicity. The Born-Oppenheimer Hamiltonian for the electrons is shown in Eq. \ref{Born-Opp}, where l are the ionic sites, Z is the nuclear charge, m is the electron mass, i is the index of the electron and j are the indices of all other electrons in the system. The first term is the kinetic energy of the electrons, the second term is the Coulomb attraction between electrons and nuclei and the third term is the Coulomb repulsion between different electrons \cite{Prasad_ch2}.
...
However, the H-F method still neglects Coulomb repulsion correlation effects that arise due to electrons moving in such a way to avoid each other. It includes some correlation effects between parallel spin electrons from the explicit treatment of exchange interactions, but neglects correlations between opposite spin electrons. The Coulomb correlations reduce the exchange interaction between electrons with parallel spins, so the H-F method overestimates the strength of the exchange interaction. The H-F method is better suited to compact systems such as atoms or molecules but describes conduction electrons in solids poorly \cite{Prasad_ch2} and, despite the approximations used, is still very computationally demanding \cite{Prasad_ch1}.
...
Plan: Start with Thomas Fermi as first electron density method (with HEG?), then HK formalism, then KS, then LDA and GGA functionals, then lead into hybrid DFT

From Prasad ch1: \cite{Prasad_ch1}
Hohenberg-Kohn proposed method to handle electron-electron interaction. In this method the ground state electron density, $\rho$, is the key quantity instead of the many-body wavefunction. This reduces the key quantity to be described by 3 degrees of freedom instead of 3N.
HK1 -- $V_{ext}$ is a functional of ground state $\rho$ therefore $\rho$ is the basic variable in this method
HK2 -- $E_{tot}$ is minimised for the correct ground state $\rho$ therefore the variational principle can be used to obtain $E_{tot}$

Hohenberg-Kohn theorem (Etot is min for correct GS electron density) gives variational principle as a method to obtain Etot

Starting point: http://iopscience.iop.org/article/10.1238/Physica.Topical.109a00009/pdf

+ York comp QM notes? + Phil Hasnip's notes (focus on ground state at use DFT)

+ Richard Martin Ch6-9 \cite{RichardMartin_Ch6} \cite{RichardMartin_Ch7} \cite{RichardMartin_Ch8} \cite{RichardMartin_Ch9}

LDA (good for metals), GGA (improvement but still underestimates band gap... lead into hybrid-DFT section)
