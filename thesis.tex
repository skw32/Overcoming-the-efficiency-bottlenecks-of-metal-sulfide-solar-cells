...
It is worth noting that the concept of the `vacuum level' here is different to that used in theoretical materials modelling (discussed later in section \ref{band_alignment_methods}) where the `vacuum level' is considered to be the energy of an electron infinitely separated from the material.
...
\begin{equation}d \boldsymbol{r}^N = \mathrm{d}x_1 \mathrm{d}y_1 \mathrm{d}z_1 \mathrm{d}x_2 \mathrm{d}y_2 \mathrm{d}z_2... \mathrm{d}x_N \mathrm{d}y_N \mathrm{d}z_N.\end{equation}\label{dr}
