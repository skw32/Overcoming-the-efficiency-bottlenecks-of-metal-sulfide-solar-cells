...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.95\textwidth]{figures/atomic_radial_wavefunction.png}
    \caption{Radial wavefunction, $r\psi_l(r)$, of Mn in the $3d^{5\uparrow}4s^2$ state showing all atomic orbitals. Taken from Ref. \citenum{RichardMartin_Ch10}.}
...
The exact, analytical solution of the Schr{\"o}dinger equation for H gives us wavefunctions corresponding to the familiar 1s, 2s, 2p, etc. atomic orbitals. Fig. \ref{atomic_radial_wavefunction} shows the calculated radial wavefunction with all orbitals for a more complicated atomic system, Mn. Accurate description of the electronic structure of atoms is a vital starting point for computing the properties of valence electrons in molecules or solids, which predominantly determine the bonding in the material. 
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.65\textwidth]{figures/aims_NACBF.png}
...
In the Fritz Haber Institute ab initio molecular simulations (FHI-aims) software package \cite{FHI-aims}, numeric atom-centred orbital (NAO) basis functions, such as that shown in equation \ref{aims_basis_set}, are the basis set used. This basis set for each atomic species has been established from iterative automated construction based on dimers \cite{FHI-aims_slides}. The radial shape, $u_i(r)$, is numerically tabulated and so fully flexible, enabling the creation of optimised element-dependent basis sets that are as compact as possible to minimise computational expense. A schematic represenatation of a NAO is shown in Fig. \ref{aims_NACBF}.
\begin{equation}\label{aims_basis_set}
\phi_{NAO}(\mathbf{r}) = \frac{u_i(\mathbf{r})}{r} Y_{lm}(\Omega)
\end{equation}
The NAO basis set used in FHI-aims is localised and highly scalable for use on modern high-performance computing (HPC) systems with multiple processors. The localised nature of the basis set has the benefit of no additional computational cost for vacuum gaps in the simulation unit cell, such as in the slab model in Fig. \ref{slab_PBCs}. This implementation of electronic structure calculations is an example of an `all-electron’ approach, which has benefitted greatly from advances in HPC technology. However, other highly-successful and computationally efficient approaches for electronic structure calculations of solids have been developed, largely prior to the HPC boom, that are not all-electron but have demonstrated accurate calculated chemical properties.

In the Vienna Ab-initio Simulation Package (VASP) \cite{VASP} plane waves (PW), as shown in Eq. \ref{vasp_basis_set}, are used as the basis set.
\begin{equation}\label{vasp_basis_set}
\phi_{PW}(\mathbf{r}) = ce^{i(\mathbf{k}+\mathbf{G}).\mathbf{r}}
\end{equation}

pseudopotential approx...

RM pseudopotential notes (ch11 summary): replace strong Coulomb potential of the nucleus and effects of tightly bound core electrons by an effective ionic potential acting on valence electrons, construct pseudopotentials from atomic wavefunctions and apply to solids… 

c.f. Prasad notes: pg 112 \cite{Prasad_ch5} rapid oscillations of wavefunction in core region gives poor convergence of plane wave expansion of wave function vs. loosely bound valence electrons where wavefunction is plane wave like, pg 125 \cite{Prasad_ch6} for justification of frozen core pseudopotentials

From Chris’ thesis – pseudopotentials
Core electrons only accurately described by high f plane waves, hence computationally expensive to compute with plane waves, core electrons not usually involved in bonding and hence contribute little electronic information in the solution for solids, therefore pseudopotentials smooth over/ do not exactly include and just represent with a few planewaves

 ** Get fig for pseudopotentials -- (RM fig 11.6 and c.f. Prasad pg126?) **

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.9\textwidth]{figures/pseudopotential.png}
    \caption{Fig 11.2 where LHS is OPW and RHS is NCPP, showing that NCPPs are less smooth? + 11.6 for ultrasoft comparison? Relate to discussion in text of using PAW and ultrasoft instead for comp efficiency?? Taken from Ref. \citenum{RichardMartin_Ch11}.}
  \label{pseudopotential}
\end{figure}

+ importance of high-quality pseudopotentials (c.f. Adam’s thesis), Cast as OPW for eigenvalue problem with smooth part of valence function plus core-like functions, PAW approach to use pseudopotentials as operators but keep full core wavefunctions, norm-conserving vs. ultrasoft pseudopotentials (summarized on pg 206+7 + c.f. Adam’s thesis and VASP slides)

pg 144 for ultrasoft PAW \cite{Prasad_ch6}
See Prasad ch5+6 for pseudopotentials + c.f. Adam’s thesis and VASP slides for ultrasoft… (PAW, combines pseudopotentials with plane wave basis set for computational  efficiency + note important of determining high-quality pseudopotentials)

+ c.f. Ruoxi’ thesis on VASP/ pseudopotenials/ plane waves

Benefits: VASP with plane waves for solids and pseudopotentials (computational efficiency of plane waves and fourier transforms) \cite{RichardMartin_Ch11} + see vasp slides + see Phil Hasnip's slides

Nod to delta project as a way of confirming that electronic properties obtained from different software packages are consisten?
...
In this section, the process of optimizing the geometry (ionic structure) and electronic structure of a give solid using the SCF method (first discussed in the Hartree method and mentioned again in the KS formalism of DFT) for a given electron density functional will be outlined.

\begin{itemize}
\item See Volker's slide `The Kohn-Sham Equations (again)', part 2 for scf loop
...
\item Chris’ thesis S2.6.1 – geom opt + justify use of PBEsol for geom and HSE for electronic properties except for defect calcs because band gap error may make defect level much shallower such that even geom relaxation about defect is impacted (ref Lany defects review)
\item Converging scf electronic cycle (inc. figure)
\item c.f. Richard Martin fig 9.1 to transfer report appendix and other people's theses for scf implementations
\item See transfer report appendix? + check against theory section
\item c.f. Bjorn Lange’s aims slides for preconditioning, etc. for geom opt.
\end{itemize}
...
When using FHI-aims, a user can select from a hierarchy of basis (sub)sets that have been tabulated for each chemical species. The inclusion of additional radial functions for each species are grouped into `tiers' and additional radial functions are included in the basis set to reach the required accuracy for settings referred to as `light', `tight' and `really\_tight' which corresponds to basis set convergence of ??meV, ?? meV and ?? meV respectively \cite{FHI-aims_manual}. Fig. \ref{FHI-aims_tiers} shows the radial wavefunctions necessary to achieve the `tight' level of convergence for Ni. 

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.95\textwidth]{figures/FHI-aims_tiers.png}
    \caption{Ni radial functions to construct basis sets with increasingly tight convergence criteria as implemented in the software package FHI-aims \cite{FHI-aims}. Taken from Ref. \citenum{FHI-aims_slides}.}
  \label{FHI-aims_tiers}
\end{figure}

The equivalent for electronic structure calculations with VASP to the process described above for FHI-aims is to converge the cutoff energy of the plane wave basis set to achieve the level of convergence desired for the property of interest.

+ see Prasad pg145 \cite{Prasad_ch6}

** Figure showing increasing no. of plane waves to better describe the form of the wavefunction + get one of my figures of convergence of property wrt ENCUT to add to it?? **
...
Electron wavefunctions will be almost identical for values of $\mathbf{k}$ that are sufficiently close, so the wavefunctions over a region of reciprocal space can be represented by considering the wavefunction at a single \textit{k}-point. It is therefore sufficient to consider the electronic states at a finite number of \textit{k}-points in order to determine the ground state energy of the solid. This approximation is illustrated in figure \ref{k_sampling}. Bloch's Theorem enables the ground state energy to be approximately determined by considering only the number of electrons in the unit cell at a finite number of \textit{k}-points, which are chosen to sample the Brillouin zone appropriately. The choice here is a balance between more \textit{k}-points for a more accurate representation of the Brillouin zone and fewer \textit{k}-points to reduce the computational expense of the calculation \cite{bloch-thesis}.

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.95\textwidth]{figures/k_sampling_full.png}
    \caption{a) The energy dispersion relation for electrons moving in a crystal, illustrating how the function can be approximately represented by a finite number of \textit{k}-points, forming an equally-spaced mesh. Figure adapted from reference \citenum{vasp_k-sampling_slides}. b) convergence of total energy and c) optical dielectric function for the same material.}
  \label{k_sampling}
\end{figure}

Similarly, the calculated value of a property of interest should be converged with respect to the \textit{k}-grid density. Certain properties require a denser \textit{k}-grid than others. This is demonstrated in the example in Fig. \ref{k_sampling}b and c where the total energy required a $4 \times 4 \times 4$ \textit{k}-grid to achieve convergence, whereas the optical dielectric function ($\epsilon$) of the same material required a $10 \times 10 \times 10$ \textit{k}-grid.

… evenly spaced Monhorst-Pack grid as common choice and even/ odd for including or not the gamma point.

\begin{itemize}
\item Converging basis set size and \textit{k}-grid density for Brillouin zone sampling w.r.t. property of interest + see Prasad pg 145 (ENMAX and kgrid for plane wave basis sets) \cite{Prasad_ch6}
\item Adam’s thesis pg 63 and ref 26 for Monkhorst-Pack kgrid
...
\item Cross-ref VASP and FHI-aims slides
\end{itemize}
