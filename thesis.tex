...
The measured current density-voltage (J-V) curve of a solar cell is used to determine the solar to electric power conversion efficiency (PCE), $\mathrm{\eta}$. The PCE of a solar cell is the ratio of power output from the solar cell ($\mathrm{P_{MP}}$) to the power input from the Sun ($\mathrm{P_{in}}$). This is shown in Eq.~\ref{PV_efficiency}. $\mathrm{P_{MP}}$ is given in terms of the V$_\mathrm{{OC}}$ (the voltage from the J-V curve of the solar cell at J = 0) and the short-circuit current density, $\mathrm{J_{SC}}$, (the current density on the J-V curve at V = 0) in Eq.~\ref{P_MP}.
For an efficient solar cell, it is desirable to have a high $\mathrm{J_{SC}}$, a high $\mathrm{V_{OC}}$ and a FF that is as close to 1 as possible.
