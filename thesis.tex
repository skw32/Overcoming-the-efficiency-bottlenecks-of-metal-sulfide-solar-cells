...
\begin{etaremune}
\item Order-disorder transition in {\CZTS} from Monte Carlo simulations \\
...
\begin{figure}[h!]
  \centering
    ...
    \caption[a) The energy dispersion relation for electrons moving in a crystal, illustrating how the function can be approximately represented by a finite number of \textit{k}-points, forming an equally-spaced mesh. b) convergence of total energy and c) optical dielectric function for the same material.]{a) The energy dispersion relation for electrons moving in a crystal, illustrating how the function can be approximately represented by a finite number of \textit{k}-points, forming an equally-spaced mesh. Fig.~adapted from Ref.~\citenum{vasp_k-sampling_slides}. b) convergence of total energy and c) optical dielectric function for the same material.}
  \label{k_sampling}
\end{figure}
...
\begin{figure}[h!]
  \centering
    ...
    \caption[Visual descriptions of terms in the equation for the formation energy of charged defects.]{Visual descriptions of terms in the equation for the formation energy of charged defects. Fig.~taken from Ref.~\citenum{pylada}.}
  \label{pylada_eq}
\end{figure}
...
The on-lattice Monte Carlo model developed to simulate thermodynamic Cu-Zn disorder is analogous to the Ising model of a ferromagnet, descriptions of which can be found in many textbooks such as Ref.~\citenum{MC} and \citenum{MC_Landau}. 
...
\subsection{Publication: `Order-disorder transition in {\CZTS} from Monte Carlo simulations'}\label{CZTS_MC}
...
\subsection{**Outlook: Quantifying the contribution of Cu-Zn disorder using Monte Carlo simulations}
Disordered lattice configurations generated from the study presented in section \ref{CZTS_MC} could be used to quantify the possible contribution to band tailing in {\CZTS} from the potential fluctuations of due to the high concentrations of $Cu_{Zn}^{-}$ and $Zn_{Cu}^{+}$ antisite defects. 
It is worth noting that fluctuations in electrostatic potential are not the only possible contribution to band tailing that and our model cannot account for any strain effects on the band gap (shown in Fig. \ref{pankove_band_fluc}) because it is fixed on lattice. However, there are different views in the literature as to whether electrostatic potential fluctuation or band gap fluctuation, or even Cu/ Zn disorder at all, is the main contribution to the observed band tailing and performance deficit of {\CZTS} solar cells \cite{culprit, kesterite_band_tails, band_tail}. As our model considers only one contribution, we would be able to isolate and quantify this specific contribution.
...
In this section, analysis that could be performed once the datasets from Tables \ref{enargite_neutral_Ef} and \ref{enargite_charged_Ef} have been completed is discussed. In the final section of this chapter recent literature on the impact of defect physics on the performance of PV materials and defect tolerance studies are discussed. The chapter ends by outlining some hypotheses for predicting the impact of defects on device performance which could be tested with a complete dataset from the case study with {\enargite}.\\

Next just show e.g. of analysis from Chen's paper (with concentration maps, discuss transition levels and bullet point analysis, possible importance of considering defect pairs and clusters in multicomponent semiconductors + finish with defect physics hypotheses which could be tested with this case study and highlight recent PV material defect physics literature and defect tolerance studies.\\

+ refer to sc-fermi for generating concentration maps as a function of electron chemical potential.
Comment that this procedure would have to be performed for all 10 chemical potential intersection points to find the optimal experimental conditions to suppress the formation of detrimental defects?\\
...
Highlight both the challenge (very idiosyncratic property of a material with interplay of many subtle effects) but also huge reward in developing reliable predictors for the impact of defects on solar cell performance - especially given computational expense (large supercells and hse are often needed for large datasets)... and I ran out of time trying to do this during my PhD!
...
\section{Supplemental material for: `Order-disorder transitions in {\CZTS} from Monte Carlo simulations'}\label{eris1_appendix}
