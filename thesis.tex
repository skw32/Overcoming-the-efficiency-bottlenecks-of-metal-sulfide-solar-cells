...
An electrostatic field can be established by creating a junction with a gradient in the work function ($\mathrm{\Phi_W}$), electron affinity (EA) or band gap ($\mathrm{E_g}$), which are all labelled in Fig.~\ref{schottky_schematic} for both a semiconductor and a metal. 
The fields established by gradients in EA or $\mathrm{E_g}$ are usually small, whereas more substantial electric fields can be achieved by a gradient in $\mathrm{\Phi_W}$ \cite{Nelson5}.
...
\Phi_W = E_{vac} - E_F,
\end{equation}
where $E_{vac}$ and $E_F$ are both labelled in Fig.~\ref{schottky_schematic}. $E_{vac}$ is the energy of the vacuum level (where an electron is considered to be completely separated from the material). 
...
$E_F$ is the Fermi level. At temperatures above \SI{0}{K}, $\mathrm{E_F}$ is considered the energy at which there is a 50\% chance of the state being occupied. For a metal, $\mathrm{\Phi_W}$ is defined by EA (indicated in Fig.~\ref{schottky_schematic}). For a semiconductor, $\mathrm{\Phi_W}$ can be controlled by doping of the material since $\mathrm{E_F}$ is dependant upon the doping. Doping can introduce additional possible energy states into the band gap of material, therefore doping can influence the energy of the state that has a 50\% chance of the state being occupied. For instance, a semiconductor doped n-type will have $\mathrm{E_F}$ closer to the CB and has a smaller $\mathrm{\Phi_W}$ than if it was doped p-type. 
...
If the two materials have different $\mathrm{\Phi_W}$, $E_{vac}$ must then change between the two materials by an amount equal to the difference in $\mathrm{\Phi_W}$ between the two materials. 
...
\begin{figure}[h!]
  \centering
    ...
    \caption[The formation of a Schottky barrier junction between an n-type semiconductor and a metal and the splitting of the electron Fermi level ($\mathrm{E_{F,n}}$) under illumination]{The formation of a Schottky barrier junction between an n-type semiconductor and a metal and the splitting of the electron Fermi level ($\mathrm{E_{F,n}}$) under illumination. Adapted from Ref.~\citenum{Nelson5} and \citenum{band_gap_rev}.}
...
\begin{figure}[h!]
  \centering
    ...
    \caption[(a) p-n junction formed between a p-type and n-type semiconductor, in the dark and in thermal equilibrium indicating the difference in work functions ($\mathrm{\Phi_W}$) and built-in bias ($\mathrm{V_{bi}}$). b) p-n junction under illumination indicating the direction of flow of minority carriers in the case of a p-type (orange) and n-type (blue) absorber layer.]{a) p-n junction formed between a p-type and n-type semiconductor, in the dark and in thermal equilibrium indicating the difference in work functions ($\mathrm{\Phi_W}$) and built-in bias ($\mathrm{V_{bi}}$). Adapted from Ref.~\citenum{Nelson5}. b) p-n junction under illumination indicating the direction of flow of minority carriers in the case of a p-type (orange) and n-type (blue) absorber layer. Adapted from Ref.~\citenum{Nelson6}.}
  \label{p-n_schematic}
\end{figure}
...
F[n(\boldsymbol{r})] = T[n(\boldsymbol{r})] + \frac{e^2}{2}\int \int \frac{1}{4\pi \epsilon_0}\frac{n(\boldsymbol{r})n(\boldsymbol{r^{\prime}})}{|\boldsymbol{r} - \boldsymbol{r^{\prime}}|}\mathrm{d}^3r\mathrm{d}^3r^{\prime} + E_{xc}[n(\boldsymbol{r})],
\end{equation}
...
E[n(\boldsymbol{r})] = \int V_{ext}(\boldsymbol{r})n(\boldsymbol{r})\mathrm{d}^3r + T[n(\boldsymbol{r})] + \frac{e^2}{2}\int \int \frac{1}{4\pi \epsilon_0}\frac{n(\boldsymbol{r})n(\boldsymbol{r^{\prime}})}{|\boldsymbol{r} - \boldsymbol{r^{\prime}}|}\mathrm{d}^3r\mathrm{d}^3r^{\prime} + E_{xc}[n(\boldsymbol{r})] .
\end{equation}
...
\frac{\partial T[n(\boldsymbol{r})]}{\partial n(\boldsymbol{r})} + V_{ext}(\boldsymbol{r}) + \int \frac{e^2}{4\pi \epsilon_0}\frac{n(\boldsymbol{r^{\prime}})}{|(\boldsymbol{r})-(\boldsymbol{r^{\prime}})|}\mathrm{d}^3\boldsymbol{r^{\prime}} + \frac{\partial E_{xc}[n(\boldsymbol{r})]}{\partial n(\boldsymbol{r})} - \mu = 0,
\end{equation}
...
E_{xc}[n(\boldsymbol{r})] = \int \epsilon_{xc}(n(\boldsymbol{r}))n(\boldsymbol{r})\mathrm{d}^3r,
\end{equation}
where $\epsilon_{xc}(n(\boldsymbol{r}))$ is the exchange-correlation energy per particle of a uniform gas of density $n(\boldsymbol{r})$ \cite{Prasad_ch3}. A simple analytic form is known for the exchange energy of the HEG and the correlation energy of the HEG has been calculated using Monte Carlo methods in Ref.~\citenum{LDA_HEG} \cite{RichardMartin_Ch8}.
...
E_{xc}^{GGA}[ n_{\uparrow}(\mathbf{r}), n_{\downarrow}(\mathbf{r})] = \int \mathrm{d}^3r f [ n_{\uparrow}(\mathbf{r}), n_{\downarrow}(\mathbf{r}), \nabla n_{\uparrow}(\mathbf{r}), \nabla n_{\downarrow}(\mathbf{r})],
\end{equation}
...
\frac{1}{\Omega_{BZ}} \int_{BZ} F(E)\delta(E_{n\boldsymbol{k}}-E)\mathrm{d}\boldsymbol{k},    
\end{equation}
...
\begin{figure}[h!]
  \centering
    ...
    \caption[Planar average (blue line) and macroscopic average (orange line) of the potential across the finite direction of a slab model indicating the slab region, vacuum gap and surface dipole energy, $\mathrm{D_s}$.]{Planar average (blue line) and macroscopic average (orange line) of the potential across the finite direction of a slab model indicating the slab region, vacuum gap and surface dipole energy, $\mathrm{D_s}$. Plot generated using the MacroDensity python library \cite{MacroDensity}.}
  \label{MD_eg}
\end{figure}
...
The difference between the macroscopic average of the vacuum potential and the bulk-like region of the surface slab is then used to obtain the surface dipole energy, $D_s$, as shown in Fig.~\ref{MD_eg}.  The eigenvalue of the valence band maximum (VBM) of the bulk, $\epsilon_{VBM}$ and $D_s$ are then used to obtain IP as
...
To obtain the band offsets from the electron affinity rule, the vacuum level of the two materials either side of the heterojunction are aligned to the same energy, the difference between the distance between the CBM and the vacuum (EA) of each material is used to predict the conduction band offset ($E_c$), as 
\begin{equation}\label{CBO_calc}
\Delta E_c = EA_{abs} - EA_{jp},
\end{equation}
where $EA_{abs}$ denotes the electron affinity of the absorber layer and $EA_{jp}$ is that of the wider-gap heterojunction partner. If $E_g$ of each material is also taken into account then the same model can be used to predict the valence band offset ($E_v$) through the difference in the ionisation potentials ($IP = EA + E_g$) as 
...
\end{equation}
where the $E_{MP1}$ term (or the `first-order MP correction') is equivalent to the corrections applied by LG where $\epsilon$ is the static (low frequency) dielectric constant of the host crystal and $\alpha$ is the Madelung constant which is only dependant upon the shape of the host supercell. 
...
<U> = \frac{ \int \mathrm{e}^{-\frac{U(\boldsymbol{r}^N)}{k_bT}} U(\boldsymbol{r}^N) \mathrm{d}\boldsymbol{r}^N}{\int \mathrm{e}^{-\frac{U(\boldsymbol{r}^N)}{k_bT}} \mathrm{d}\boldsymbol{r}^N}  = \frac{  \int \mathrm{e}^{-\frac{U(\boldsymbol{r}^N)}{k_bT}} U(\boldsymbol{r}^N) \mathrm{d}\boldsymbol{r}^N}{Z_{NVT}} ,
\end{equation}
...
n = N\mathrm{e}^{\frac{-\Delta H}{k_BT}},
\end{equation}
...
\end{tabular}
\caption[Formation energies, $\mathrm{\Delta H_{D, q = 0}}$ of charge neutral vacancies in enargite ({\enargite}) at all 10 intersection points in the chemical potential space for enargite from Table \ref{enargite_int_points}. $\mathrm{E_{tot}}$ are total energies of the defective supercells and defects are labelled according to their spacegroup (sg) and unique structure (s) as identified by crystal symmetry using Transformer libraries.]{Formation energies, $\mathrm{\Delta H_{D, q = 0}}$ of charge neutral vacancies in enargite ({\enargite}) at all 10 intersection points in the chemical potential space for enargite from Table \ref{enargite_int_points}. $\mathrm{E_{tot}}$ are total energies of the defective supercells and defects are labelled according to their spacegroup (sg) and unique structure (s) as identified by crystal symmetry using Transformer libraries \cite{Transformer}.}\label{enargite_neutral_Ef}
\end{table}
