...
Neater version of plot from excel + describe differences...\\

Discuss need to account for screening from electronic structure in Eris... but for now will show results for on-site electrostatics potentials calculated from our Monte Carlo model where the bulk dielectric constant of perfect {\CZTS} is used to account for dielectric screening.\\

\begin{figure}[]
  \centering
    \includegraphics[width=0.85\textwidth]{figures/spatial_pot_Cu-Sn_950K_edit.png}
...
Show e.g. of 2D hist plot with region of $Zn_{Cu}^{+}$ in Cu-Sn layer and corresponding pot fluc of neighbouring Sn's and comment on scanning transmission electron microscopy measurements of clusters of $Zn_{Cu}^{+}$ \cite{Zn-on-Cu_clusters} and in Mendis work \cite{CZTS_TEM}\\
...
kde plots of potential distribution just before, during and after ODT, show how it broadens from a delta function to approx normal dist\\
...
Idea to use variance and mean of distribution to apply Gaussian broadening to electronic DOS - show e.g. of normal distributions compared to kde's? + relate back to gamma parameter in gokmen fig\\
