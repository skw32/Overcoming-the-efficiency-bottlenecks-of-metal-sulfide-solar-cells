...
Defect formation energies under specific synthesis conditions can be used to infer the likely concentrations of particular defects. 
...
The formation energy, $\Delta H_{D,q=0}$, of charge neutral point defects in a supercell can be obtained by comparing the total energy calculated for the defective supercell to that of an equivalent perfect supercell and then considering the species added to or removed from the perfect supercell when the particular defect is formed. $\Delta H_{D,q=0}$ for a charge neutral defect is shown in Eq. \ref{defect_neutral} where $E_{D,q=0}$ is the total energy of the defective supercell, $E_{host}$ is the total energy of an equivalent supercell of the perfect, bulk host crystal, $n_i$ is the number of atoms of species i added to or removed from the system when the defect is formed and $\mu_i$ is the chemical potential of species i. The chemical potential of a species i is the change in energy when one particle of type i is added to the system \cite{chem_pot}. In Eq. \ref{defect_neutral} $\mu_i$ allows us to describe the formation energy for defects in various growth conditions, such as rich or poor in particular species.
\begin{equation}\label{defect_neutral}
\Delta H_{D,q=0} = E_{D,q=0} - E_{host} - \sum_i n_i \mu_i
\end{equation}

\subsubsection{Charged point defects}
However, additional complexities arise when attempting to obtain the formation energy of a charged defect. Firstly, there is a strong and long-ranged Coulomb interaction between charged supercells in PBCs (indicated in Fig. \ref{defect_PBCs}b) and this converges slowly with increased supercell size.
Secondly, electronic structure calculations with PBCs for a charged unit cell (effectively) include a neutralising homogeneous background charge to avoid infinite charge, which is not present in the calculation for the perfect equivalent supercell. Thirdly, the charge of the defect system does not match that of reference perfect bulk. It is therefore necessary to introduce a chemical potential to account for the change in energy when electrons are added to or removed from the system when creating a defect in a given charge state \cite{freysoldt_rev}. Consequently, the expression for the defect formation energy given in Eq. \ref{defect_neutral} must be modified to that shown in Eq. \ref{defect_charged}.
\begin{equation}\label{defect_charged}
\Delta H_{D,q} = E_{D,q} - E_{host} - \sum_i n_i \mu_i + q[\epsilon_F + \epsilon_{\nu} + \Delta \nu_{0/b}] + E^q_{corr}
\end{equation}
Additional terms in Eq. \ref{defect_charged} are: $q \epsilon_F$ (electron chemical potential for energy required to add or remove electrons when charged defect forms), $q \epsilon_{\nu}$ (energy of bulk VBM) and $q \Delta \nu_{0/b}$ (term used to align the electrostatic potential of bulk and defect supercells). The terms in Eq. \ref{defect_charged} are explained pictorially in Fig. \ref{pylada_eq}.
\begin{figure}[h!]
  \centering
...
It is therefore crucial to estimate the magnitude and decay behavior of the different effects.
...
or call - a posteriori Finite-size corrections to formation energy of charged defects?
Outline of some (but acknowledge that not all are presented here) post-calculation finite-size correction schemes for the supercell method. (FNV, LZ, extrapolation + Kumagai/ Oba?)

Discuss different correction steps (see pylada paper) and different schemes (see Komsa paper?). + see JC talk slides\\

Mention importance of band gap error in standard DFT for defect calculations (see Lany defects review) \cite{Lany_defects}



\cite{freysoldt_rev}, notes from JC talk:

pa
Supercell calculations for charged systems must include compensating background charge (electrostatic energy of a system with a net charge in the unit cell diverges) 
Usually homogeneous background is included (equivalent to setting the average electrostatic potential to zero)
The formation energy of a charged defect depends on the Fermi level 
Referenced to the bulk VBM (depends on the average electrostatic potential in the bulk)
The long-range nature of the Coulomb potential precludes establishing an absolute reference for the electrostatic potential 
 hence need to align the average electrostatic potential in the defect supercell with that in the bulk

ic:
The unphysical electrostatic interaction of the defect with its periodic images and the constant background makes a spurious contribution to the calculated energy of the system
The magnitude of these interactions can be estimated from the Madelung energy of an array of point charges with neutralizing background (Leslie and Gillan, 1985)
The interaction decays asymptotically as q2/L (Leslie and Gillan, 1985; Makov and Payne, 1995), where L is a representative supercell dimension
Makov and Payne proved for isolated ions that the quadrupole moment of the charge distribution gives rise to a further term scaling as L−3 (Makov and Payne, 1995)
...
Introduce as one method for simulating defects and disorder on scales much larger than possible with electronic structure calculations. Above for point defects and pairs in {\CZTS} has been performed already \cite{defects_Chen}, but evidence of high concentrations of disorder **add exptl refs**.

\subsection{The Monte Carlo method}
Introduce general method (see transfer report + textbooks)\\
...
\subsubsection{Equilibration and finite-size effects }\label{equilibration}
...
\subsubsection{Defects beyond the dilute limit in {\CZTS}}
\begin{figure}[h!]
  \centering
...
\subsubsection{Custom on-lattice Monte Carlo code for Cu-Zn disorder in {\CZTS}}
\begin{figure}[h!]
  \centering
...
+ Give pairwise Hamiltonian here?

+ Mention code applied in study in Ch4.2

%\subsubsection{Quantification of disorder using pair correlation functions}\label{RDF_methods}
...
\section{Band alignment for solar cell p-n heterojunctions}\label{sulfosalt_band_alignment}
